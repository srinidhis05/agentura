"""Parse aspora.config.yaml → SkillConfig."""

from pathlib import Path

import yaml

from aspora_sdk.types import SkillConfig


def load_config(config_path: Path) -> SkillConfig:
    """Load and validate an aspora.config.yaml file."""
...
def find_config(skill_dir: Path) -> Path:
    """Find aspora.config.yaml walking up from skill_dir."""
    current = skill_dir.resolve()
    while current != current.parent:
        candidate = current / "aspora.config.yaml"
        if candidate.exists():
            return candidate
        current = current.parent
    raise FileNotFoundError(
        f"No aspora.config.yaml found in or above {skill_dir}"
    )
