"""Aspora Skill Executor — FastAPI server wrapping SDK functions."""
...
KNOWLEDGE_DIR = Path(os.environ.get("ASPORA_KNOWLEDGE_DIR", ".aspora"))
...
app = FastAPI(title="Aspora Skill Executor", version="0.1.0")
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "https://srinidhi-fe.knowyourshit.xyz",
    ],
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def _load_domain_config(domain_dir: Path) -> dict[str, Any]:
    """Load and cache domain aspora.config.yaml data (cached with TTL). Returns empty dict on failure."""
    key = str(domain_dir)
    now = time.time()
    ...
    # Check all skill subdirs for aspora.config.yaml (it's at skill level)
    for child in domain_dir.iterdir():
        cfg_path = child / "aspora.config.yaml"
        if cfg_path.exists():
            try:
...
async def execute(domain: str, skill_name: str, req: ExecuteRequest):
    ...
    # Compose system prompt: DOMAIN.md + reflexion + SKILL.md
    prompt_parts = []
    ...
    skill_config_path = root / "aspora.config.yaml"
...
def correct(domain: str, skill_name: str, req: CorrectRequest):
    ...
    """
    from aspora_sdk.cli.correct import (
        _generate_reflexion,
        _load_execution,
        _store_correction,
        _update_guardrails,
...
def _update_execution_outcome_to_pending_approval(domain: str, skill_name: str) -> None:
    ...
    for p in [
        ...
        Path.cwd() / ".aspora" / "episodic_memory.json",
        Path("/skills") / ".aspora" / "episodic_memory.json",
    ]:
        if not p.exists():
            continue
        try:
...
def _load_knowledge_file(name: str) -> dict:
    ...
    candidates = [
        KNOWLEDGE_DIR / name,
        Path.cwd() / ".aspora" / name,
        Path("/skills") / ".aspora" / name,
    ]
    for p in candidates:
        if p.exists():
            try:
...
def list_executions(skill: str | None = None, outcome: str | None = None):
    """List execution history from episodic memory."""
...
def get_execution(execution_id: str):
    """Get execution detail with linked corrections and reflexions."""
    data = _load_knowledge_file("episodic_memory.json")
    entries = data.get("entries", [])
    entry = next((e for e in entries if e.get("execution_id") == execution_id), None)
    if not entry:
...
def get_analytics():
    """Aggregate metrics across all executions, corrections, and reflexions."""
    exec_data = _load_knowledge_file("episodic_memory.json")
    entries = exec_data.get("entries", [])

    corr_data = _load_knowledge_file("corrections.json")
    corrections = corr_data.get("corrections", [])

    refl_data = _load_knowledge_file("reflexion_entries.json")
    reflexions = refl_data.get("entries", [])
...
def list_reflexions(skill: str | None = None):
    """List all reflexion entries across all skills."""
...
def list_corrections(skill: str | None = None):
    """List all corrections across all skills."""
...
def get_knowledge_stats():
    """Learning metrics across all skills."""
    corr_data = _load_knowledge_file("corrections.json")
    corrections = corr_data.get("corrections", [])
...
def semantic_search(domain: str, skill_name: str, req: SemanticSearchRequest):
    """Semantic search across corrections and reflexions for a skill.

    Uses mem0 vector search when available, falls back to exact match.
    """
    skill_path = f"{domain}/{skill_name}"
    try:
...
def get_memory_status():
    """Memory backend status — is mem0 active, what's stored, what's tracked."""
    status = MemoryStatus()
    ...
    try:
        ...
        if isinstance(store, Mem0Store):
            ...
            status.llm_provider = "anthropic" if os.environ.get("ANTHROPIC_API_KEY") else "openai"

            # Count entries by type
            try:
                execs = store.get_executions()
                status.execution_memories = len(execs)
            except Exception:
                pass
            try:
                corrs = store.get_corrections()
                status.correction_memories = len(corrs)
            except Exception:
                pass
            try:
                refls = store.get_all_reflexions()
                status.reflexion_memories = len(refls)
            except Exception:
                pass
    ...
    # If mem0 not available, count from JSON
    if status.backend == "json":
        status.execution_memories = len(exec_data.get("entries", []))
        corr_data = _load_knowledge_file("corrections.json")
        status.correction_memories = len(corr_data.get("corrections", []))
        refl_data = _load_knowledge_file("reflexion_entries.json")
        status.reflexion_memories = len(refl_data.get("entries", []))
        status.total_memories = (
            status.execution_memories
            + status.correction_memories
            + status.reflexion_memories
...
def memory_search(req: CrossDomainSearchRequest):
    """Cross-domain semantic search across ALL skill memories."""
    # Gather all skills
    exec_data = _load_knowledge_file("episodic_memory.json")
    skills_set: set[str] = set()
    for e in exec_data.get("entries", []):
    ...
    if not all_results:
        ...
        for e in exec_data.get("entries", []):
            text = _json.dumps(e)
            score = _word_score(text) * 0.5  # Max 0.5 for executions
            if score > 0.1:
                all_results.append({
                    "type": "execution",
                    "skill": e.get("skill", ""),
        ...
        for c in corr_data.get("corrections", []):
            text = _json.dumps(c)
            score = _word_score(text) * 0.85  # Max 0.85 for corrections
            if score > 0.1:
                all_results.append({
                    "type": "correction",
                    "skill": c.get("skill", ""),
        ...
        for r in refl_data.get("entries", []):
            text = _json.dumps(r)
            score = _word_score(text) * 0.95  # Max 0.95 for reflexions
            if score > 0.1:
                all_results.append({
                    "type": "reflexion",
                    "skill": r.get("skill", ""),
...
class PromptAssemblyResponse(BaseModel):
    """Shows how a skill's system prompt is assembled from 3 layers."""
    skill: str
    domain_context: str = ""
...
def get_prompt_assembly(domain: str, skill_name: str):
    """Show how a skill's system prompt is assembled from DOMAIN.md + reflexions + SKILL.md."""
    ...
    if tests_passed > 0:
        ...
        if updated:
            ...
            for p in [
                ...
                Path.cwd() / ".aspora" / "reflexion_entries.json",
            ]:
                if p.exists():
...
def list_events(domain: str | None = None, event_type: str | None = None, limit: int = 50):
    """Unified event stream across all platform activity."""
...
def get_platform_health():
    ...
    db_status = "healthy" if (KNOWLEDGE_DIR / "episodic_memory.json").exists() or (Path.cwd() / ".aspora" / "episodic_memory.json").exists() else "no_data"
...
def create_skill(req: CreateSkillRequest):
    """Create a new skill scaffold via the API (same as `aspora create skill`)."""
    ...
    skill_files: dict[Path, str] = {
        ...
        skill_root / "aspora.config.yaml": "aspora.config.yaml.j2",
    ...
    for filepath, template_name in all_files.items():
        ...
        if filepath.name == "aspora.config.yaml" and req.model:
...
def main():
    """Entry point for aspora-server command."""
    import uvicorn
