"""aspora correct <domain>/<name> — Capture correction and auto-generate tests."""
...
@click.option("--execution-id", required=True, help="Execution ID to correct (from aspora run output).")
...
def correct(skill_path: str, execution_id: str, correction: str, skills_dir: str):
    ...
    if not execution:
        ...
        console.print("[dim]Run a skill first with 'aspora run' to generate executions.[/]")
        raise SystemExit(1)
...
def _print_summary(console, skill_path, correction_id, reflexion_id, deepeval_file, promptfoo_file):
    ...
    console.print(Panel(
        ...
        f"  aspora run {skill_path}\n\n"
        f"[dim]Run tests to verify:[/]\n"
        f"  aspora test {skill_path}",
