"""aspora apply -f <path> — Declarative skill deployment (like kubectl apply)."""
...
def apply(path: str, dry_run: bool):
    ...
    Scans the path for aspora.config.yaml files, validates each skill,
    and registers them with the running gateway.

    Examples:
      aspora apply -f skills/ecm/order-details/    # Single skill
      aspora apply -f skills/ecm/                   # Entire domain
      aspora apply -f skills/                        # Everything
    """
    console = Console()
    target = Path(path)

    # Find all config files under path
    configs = sorted(target.rglob("aspora.config.yaml"))
    if not configs:
        console.print(f"[yellow]No aspora.config.yaml found under {target}[/]")
        return
    ...
    for config_path in configs:
        ...
        for skill_ref in config.skills:
            ...
            # Deploy to gateway
            try:
                from aspora_sdk.cli.gateway import _client
                with _client() as c:
