...
agentura create skill ecm/new-skill     # Scaffold skill
agentura validate ecm/order-details     # Validate structure
agentura run ecm/order-details          # Execute locally
agentura run ecm/order-details --dry-run # Validate without calling model
agentura correct ecm/order-details      # Submit correction → auto-gen tests
agentura test ecm/order-details         # Run tests
```

**Needed (gateway operations)**:
```bash
agentura get skills                     # List from gateway
agentura get skills -d ecm              # Filter by domain
agentura get executions                 # List execution history
agentura describe skill ecm/order-details   # Full detail view
agentura logs EXEC-20260218120000       # Reasoning trace
agentura apply -f skills/ecm/           # Deploy domain to gateway
